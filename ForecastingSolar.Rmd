...
logsolarplot + ggthemes::theme_tufte()
```
  
We see the forecast does a fair job at predicting the actual solar energy production.  
  
For fun, we can extrapolate, and forecast the energy output 15 years in the future:
...
#create dataframe for the 5 year forecast
...
```{r, echo=FALSE, message=FALSE, warning=FALSE}
#plot generation sources over time
forecast15plot <- ggplot(data = forc5df) + 
...
forecast15plot + ggthemes::theme_tufte()
```
