...
output:
  prettydoc::html_pretty:
    ...
    highlight: github
---
[Made with R markdown.  See the code on my GitHub Page](https://github.com/DavidNoonan/Forecasting-Energy-Generation)  
  

This is a timeseries analysis of the total U.S. solar energy generation. The data is from the U.S. Energy Information Administration, consisting of monthly totals from 1973 to June 2017. Here we will fit an ARIMA model, and make 12-month and 5-year forecasts.
...
The data contains monthly totals from 1973-2017 by energy source, with units in quadrillions of British Thermal Units. I converted the units to gigawatt-hours, which are more appropriate for electricity.
...
If we look carefully, we see solar energy is one of the barely visible squiggles on the bottom right of the plot. It is dwarfed by the major energy sources: natural gas, coal, and oil. Nevertheless, solar is experiencing exponential growth. We take a closer look below, along with wind energy, another renewable source with a similar growth pattern:
...
Both sources are growing exponentially, beginning roughly in year 2000 for wind and 2010 for solar. Since solar begins its growth phase at about 2010, we will focus on that time frame to build a model.
  
  
We make this easier to model using a logarithmic transformation, which means that we will model the growth rate. Below is a plot of the log transformed solar generation:  
...
What we see here is a remarkably constant trend that oscillates with the seasons. We need to remove this trend to build an ARIMA model, because ARIMA models assume a stationary process (the mean and variance do not change over time). We can do this with differencing, a transformation where the previous value is subtracted from each observation. Below is the differenced series:
...
Differencing did a great job of removing the trend. Now that we have what looks like a stationary process, we can begin fitting a model. Our first step is looking at auto-correlation (ACF) and partial-auto-correlation (PACF) plots to get an idea of candidate model parameters we can use:  
